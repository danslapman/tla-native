#!/usr/bin/env bash
# build-linux.sh - Build tlc2.TLC as a GraalVM native executable (Linux)
#
# Usage:
#   ./build-linux.sh           # build native image (uses graal-config/ if present)
#   ./build-linux.sh --trace   # run TLC with native-image-agent to collect config
#   ./build-linux.sh --build   # same as default
#   BUILD_MEMORY=12g ./build-linux.sh  # customize build-time heap (default: 8g)
...
do_trace() {
  ...
  if [[ ! -f "${TRACE_SPEC}.tla" ]]; then
    ...
    echo "       HelloWorld.tla must be alongside build-linux.sh." >&2
    exit 1
  fi
  ...
  ls -la "${CONFIG_DIR}/"
  echo ""
  echo "    Run './build-linux.sh' to build the native image."
}
...
do_build() {
  ...
  local -a COMMON_FLAGS=(
    ...
    # Use G1GC as the garbage collector in the generated native binary at runtime.
    # (Oracle GraalVM native image defaults to Serial GC; G1GC is available in Oracle GraalVM.)
    "--gc=G1"
  ...
  if [[ -f "${METADATA_FILE}" ]]; then
    ...
    if [[ -d "${CONFIG_DIR}" ]]; then
      ...
      echo "    Run './build-linux.sh --trace' first for best results."
      echo ""
    fi
    FLAGS+=("-jar" "${JAR}")
  fi
